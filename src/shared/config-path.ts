...
export function getUserConfigDir(): string {
  if (process.platform === "win32") {
    ...
    const crossPlatformConfigPath = path.join(crossPlatformDir, "opencode", "ofx-code.json")

    const appdataDir = process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming")
    const appdataConfigPath = path.join(appdataDir, "opencode", "ofx-code.json")
...
/**
 * Returns the full path to the user-level ofx-code config file.
 */
export function getUserConfigPath(): string {
  return path.join(getUserConfigDir(), "opencode", "ofx-code.json")
}

/**
 * Returns the full path to the project-level ofx-code config file.
 */
export function getProjectConfigPath(directory: string): string {
  return path.join(directory, ".opencode", "ofx-code.json")
}

/**
 * Detects external plugins that may conflict with ofx-code features.
...
/**
 * Known notification plugins that conflict with ofx-code's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[ofx-code] External notification plugin detected: ${pluginName}

⚠️  Both ofx-code and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   ofx-code's session-notification has been auto-disabled.

   To use ofx-code's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in ofx-code.json`
}

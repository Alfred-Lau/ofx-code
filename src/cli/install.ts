...
function formatConfigSummary(config: InstallConfig): string {
  ...
  const sisyphusModel = config.hasClaude
    ? "claude-opus-4-5"
    : config.hasCopilot
    ? "github-copilot/claude-opus-4.5"
    : "glm-4.7-free";
  const oracleModel = config.hasChatGPT
    ? "gpt-5.2"
    : config.hasCopilot
  ...
  const frontendModel = config.hasGemini
  ...
  lines.push(
    `  ${SYMBOLS.bullet} Sisyphus     ${SYMBOLS.arrow} ${color.cyan(
      sisyphusModel
    )}`
  );
  lines.push(
    `  ${SYMBOLS.bullet} Oracle       ${SYMBOLS.arrow} ${color.cyan(
      oracleModel
    )}`
  );
  lines.push(
    `  ${SYMBOLS.bullet} Librarian    ${SYMBOLS.arrow} ${color.cyan(
      librarianModel
    )}`
  );
  lines.push(
    `  ${SYMBOLS.bullet} Frontend     ${SYMBOLS.arrow} ${color.cyan(
      frontendModel
    )}`
  );
    ...
    options: [
      {
        value: "no" as const,
        label: "No",
        hint: "Oracle will use fallback model",
      },
      {
        value: "yes" as const,
        label: "Yes",
        hint: "GPT-5.2 for debugging and architecture",
      },
    ],
...
async function runNonTuiInstall(args: InstallArgs): Promise<number> {
  ...
  if (!validation.valid) {
    ...
    console.log();
    printInfo(
      "Usage: bunx oh-my-opencode install --no-tui --claude=<no|yes|max20> --chatgpt=<no|yes> --gemini=<no|yes> --copilot=<no|yes>"
    );
    console.log();
    return 1;
  }
  ...
  printStep(step++, totalSteps, "Adding ofx-code plugin...");
  ...
  printSuccess(
    `Plugin ${isUpdate ? "verified" : "added"} ${SYMBOLS.arrow} ${color.dim(
      pluginResult.configPath
    )}`
  );
  ...
  if (config.hasGemini || config.hasChatGPT) {
    ...
    printSuccess(
      `Auth plugins configured ${SYMBOLS.arrow} ${color.dim(
        authResult.configPath
      )}`
    );
    ...
    printSuccess(
      `Providers configured ${SYMBOLS.arrow} ${color.dim(
        providerResult.configPath
      )}`
    );
  } else {
    step += 2;
  ...
  printStep(step++, totalSteps, "Writing ofx-code configuration...");
  ...
  if (
    (config.hasClaude || config.hasChatGPT || config.hasGemini) &&
    !args.skipAuth
  ) {
    ...
    if (config.hasClaude) {
      console.log(
        `  ${SYMBOLS.arrow} ${color.dim("opencode auth login")} ${color.gray(
  ...
  printBox(
    `${color.bold("Pro Tip:")} Include ${color.cyan(
      "ultrawork"
  ...
  console.log(
    `${SYMBOLS.star} ${color.yellow(
      "If you found this helpful, consider starring the repo!"
    )}`
  );
  console.log(`  ${color.dim("gh repo star Alfred-Lau/ofx-code")}`);
  console.log();
...
export async function install(args: InstallArgs): Promise<number> {
  ...
  if (isUpdate) {
    const initial = detectedToInitialValues(detected);
    p.log.info(
      `Existing configuration detected: Claude=${initial.claude}, ChatGPT=${initial.chatgpt}, Gemini=${initial.gemini}`
    );
  }
  ...
  s.start("Adding ofx-code to OpenCode config");
  const pluginResult = addPluginToOpenCodeConfig();
  if (!pluginResult.success) {
  ...
  s.start("Writing ofx-code configuration");
  const omoResult = writeOmoConfig(config);
  if (!omoResult.success) {
  ...
  if (
    ...
    if (config.hasClaude) {
      steps.push(
        `${color.dim("opencode auth login")} ${color.gray(
          "(select Anthropic → Claude Pro/Max)"
        )}`
      );
    }
  ...
  p.note(
    `Include ${color.cyan("ultrawork")} (or ${color.cyan(
      "ulw"
    )}) in your prompt.\n` +
      `All features work like magic—parallel agents, background tasks,\n` +
      `deep exploration, and relentless execution until completion.`,
  ...
  p.log.message(`  ${color.dim("gh repo star Alfred-Lau/ofx-code")}`);

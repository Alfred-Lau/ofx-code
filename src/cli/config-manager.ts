...
export function generateOmoConfig(
  ...
  const config: Record<string, unknown> = {
    $schema:
      "https://raw.githubusercontent.com/Alfred-Lau/ofx-code/master/assets/ofx-code.schema.json",
  };
  ...
  if (!installConfig.hasClaude) {
    agents["Sisyphus"] = {
      model: installConfig.hasCopilot
  ...
  if (installConfig.hasGemini) {
    ...
    const fallbackModel = installConfig.hasClaude
      ? "anthropic/claude-opus-4-5"
      : "opencode/glm-4.7-free";
...
export function writeOmoConfig(
  ...
  try {
    ...
    return {
      ...
      error: formatErrorWithSuggestion(err, "write ofx-code config"),
    };
  }
}
...
export function detectCurrentConfig(): DetectedConfig {
  ...
  result.isInstalled = plugins.some((p) => p.startsWith("ofx-code"));
  ...
  result.hasChatGPT = plugins.some((p) =>
    p.startsWith("opencode-openai-codex-auth")
  );
  ...
  try {
    ...
    const hasAnyCopilotModel = Object.values(agents).some((agent) =>
      agent?.model?.startsWith("github-copilot/")
    );
